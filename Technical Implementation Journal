...
⚙️ DVWA Deployment & Configuration
...
Enabling Advanced DB Logging
To get deeper forensic data, I enabled the Slow Query Log by editing /etc/mysql/mariadb.conf.d/50-server.cnf:

Ini, TOML
# Added to the end of the configuration
slow_query_log = 1
slow_query_log_file = /var/log/mysql/mysql-slow.log
long_query_time = 1
log_queries_not_using_indexes = 1
🧪 Live SQLi Testing & Evidence Collection
To build our detections, we need raw evidence. I ran a controlled SQL Injection test through the browser using a classic payload:
' OR '1'='1' --

Capturing the Data
While the attack was running, I monitored the logs in real-time:

Bash
sudo tail -F /var/log/apache2/access.log /var/log/apache2/error.log /tmp/mysqld-general.log
Storing Evidence for SIEM Ingestion
Once the test was done, I moved the relevant logs into a safe folder to be compressed and shipped to the collector later.
...
sudo cp /tmp/mysqld-general.log ~/dvwa-evidence/mysql-general.log

# Compress the evidence
tar -czvf ~/dvwa-evidence.tgz -C ~ dvwa-evidence
This evidence archive is now ready for future SIEM ingestion where we will build out our visualization and alerting rules.
